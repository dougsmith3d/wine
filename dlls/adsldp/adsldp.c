                                           ...
                                           DWORD count, PADS_SEARCH_HANDLE res)
{
    ...
    else if (!object)
    {
         ...
         * default naming context read from rootDSE; use it as the search base. A subtree search
         * rooted there also returns continuation references to the forest's sibling naming
         * contexts (Configuration, *DnsZones); OpenLDAP chases them and fails the whole search
         * with LDAP_OPERATIONS_ERROR, whereas Windows ADSI (ADS_CHASE_REFERRALS_EXTERNAL) does
         * not chase these subordinate referrals. Match ADSI on this dedicated connection only. */
